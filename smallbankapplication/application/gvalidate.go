...
func (BCstate *BlockchainState) GValidate(s []SmallBankTransaction, GE [][]GraphEdge, groupNUm int, version map[string]AccountVersion, ch chan bool) {
	order, m := Dfs(GE, groupNUm)
	lG := len(order)
